...
When used alongside the X3 Plugin Manager, run X3 Customizer 3.05.1 second, after the plugin manager is closed, since the plugin manager generates a TWareT.pck file when closed that doesn't capture changes in TWareT.txt.
 ...
 * Prevent_Accidental_Spacefly_Swarms
      ...
      Prevents spaceflies from spawning swarms when created by a script using the 'create ship' command. Aimed at mods such as Improved Races 2.0 and XRM which create all ships, record data, and then destroy the ships, leaving behind the spacefly swarm, with swarms accumulating over time and drastically slowing the game down.
