...
@Check_Dependencies('TFactories.txt', 'WareTemplate.xml')
def Add_More_Factory_Sizes(
    ...
    * warn_on_waretemplate_bugs:
    ...
    if _cleanup:
        for file_path in [dummy_template_path, director_loader_path]:
            if os.path.exists(file_path):
                os.remove(file_path)
        return
    # Quick double check the maps folder exists.
    ...
    race_type_name_size_factory_dict = defaultdict(
        lambda: defaultdict(
            lambda: defaultdict(
                dict)))
    for factory_dict in Load_File('TFactories.txt'):
    ...
    waretemplate_xml = Load_File('WareTemplate.xml')
    # Parse the xml.
    ...
    Add_Entries_To_T_File('TFactories.txt', new_factories_list)
    ...
    if print_count:
        Write_Summary_Line('Number of new factories added: {}'.format(len(new_factories_list)))
