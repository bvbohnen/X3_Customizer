...
Note: problem eventually tracked down to:
    A) "create ship" command spawning a spacefly swarm instead of just 1,
        with active scripts, an undocumented and unexpected behavior.
    B) Improved Races 2.0, Salvage Command Software, maybe other mods
        (Gazz improved missile defense appears likely), will create a
        spacefly and destroy it, leaving behind a swarm.
'''
...
Some background:
    ...
    Editing the code jumped to will chance how script commands work,
    while editing the jump table can redirect from one command handler
    to another (though this is only safe if the commands match up in
    expected input and return args).
...
Update:
    ...
    both Improved Races 2.0 and SCS generating the lingering
...
Note on LU, TC:
    ...
    Some TC support has been added; LU support could be added if needed
    fairly easily (checking ~5 addresses).

    The swarm suppression patch further below should work okay for LU/TC
    as-is.
'''
...
def Kill_Spaceflies():
    ...
    spacefly swarms generated by Improved Races 2.0 or Salvage
    Command Software or other mods which accidentally add excess
    spaceflies through the "create ship" command, causing accumulating
    slowdown (eg. 85% SETA slowdown after 10 game days).
    ...
    # final approach (7) will get kept as active, supported code.
    # TODO: maybe delete this section if it won't be useful as reference.
    ...
    #  in a kill delay.
    # Note: since this uses hard-coded function addresses, it is not portable
    #  across obj versions.
    ...
    if Settings.target_base_tc:
        ...
        IsDisabled         = '00009720'
        TI_Delay           = '00002561'
        JumpOutOfExistence = '0000A432'
        # Code jump labels inside the RunScript handler.
        return_label       = '000715A4'
        IsDocked_label     = '000379DC'
    else:
        # AP 3.3 addresses.
        # Function addresses.
        IsDisabled         = '00007143'
        TI_Delay           = '00002A99'
        JumpOutOfExistence = '0000B2AA'
        # Code jump labels inside the RunScript handler.
        return_label       = '000785F1'
        IsDocked_label     = '00038982'
...
def Prevent_Accidental_Spacefly_Swarms(
    ...
    Aimed at mods such as Improved Races 2.0 and SCS, which create
    and destroy a spacefly but accidentally leave behind a swarm
    with active scripts, causing spacefly accumulation and game
    slowdown.
    '''
    ...
    Note on LU, base TC:
        ...
        this patch should work on LU or TC just fine with enough of
        a pattern to match.
    '''
