...
TODO: switch many of the subtype checks to using class type, which will be more
accurate, eg. a fighter drone is subclass M5 but has a unique object type
(not M5), so using object type may be a better filter for actual M5s.
Also, the xrm goner M6 (TLS) is listed as subtype GO, object type M6, so
filtering by object type would capture it better.
Exact use of subtype is currently unknown; class should map to the obj
code actual class.
