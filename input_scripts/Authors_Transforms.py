...
# Select if this is modifying vanilla or XRM or LU or FL.
#version = 'Vanilla'
#version = 'XRM'
#version = 'LU'
#version = 'Mayhem'
...
if version == 'Vanilla':
    Set_Path(
        path_to_x3_folder = r'D:\Games\Steam\SteamApps\common\x3 terran conflict',
    ...
    Set_Path(
        path_to_x3_folder = r'D:\Games\Steam\SteamApps\common\x3 terran conflict LU Mayhem',
    )
elif version == 'TC':
    Set_Path(
        path_to_x3_folder = r'D:\Games\Steam\SteamApps\common\x3 terran conflict without AP',
        enable_TC_mode = True
    )
elif version == 'FL':
    Set_Path(
    ...
    if Vanilla or FL:
        Adjust_Job_Count(
            job_count_factors = [
                ...
                # Add some more basic goods traders.
                ('Energy ', 1.5),
                ('Mineral ', 1.5),
                ('Ore ', 1.5),
        ...
        # Maybe tweak respawn timers, if problematic.
        # Mostly testing this for now.
        Adjust_Job_Respawn_Time(
            time_adder_list = [
                # Just add a few minutes to everything.
                ('*', 5),
...
#####################################################
# A shared damage scaling term between t_weapons and t_ships.
...
def T_Weapons():
    ...
    # TODO: check if PAL still needs these edits.
    if Vanilla or FL:
...
def T_Ships():
    ...
    if FL:
        ...
        if 0:
            # TODO: consider which variants to include. Traders are already
            # pretty variant heavy, so maybe just combat.
            #Add_Ship_Trade_Variants(print_variant_modifiers = True, 
            #                        print_variant_count = True)
            Add_Ship_Combat_Variants(
                print_variant_count = True
        ...
        # TODO: check if this still makes sense for FL speeds.
        Adjust_Ship_Speed(
            adjustment_factors_dict = {
                # Bump up interceptors about 30%, to better distinguish their role from
                #  fighters (which otherwise have a lot of speed overlap).
...
def T_Scripts():
    ...
    # The following don't work for LU/FL.
    if Vanilla or XRM:
